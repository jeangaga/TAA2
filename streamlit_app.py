...
with tabs[3]:
    ...
    if st.session_state.scenario_book is None:
        ...
        #
        # Columns:
        #   Strategy          — sleeve label, with a TAA row pinned to
        #                       the bottom (the eye sees the strategy
        #                       breakdown first, total last).
        #   Ann.Vol           — standalone annualised vol of the sleeve
        #                       return series.
        #   Risk Contrib %    — sleeve's share of TAA vol, rebased so
        #                       strategies sum to 100%. Marginal-vol
        #                       contributions normalised by the total
        #                       (sourced from `risk.compute_risk_contrib`'s
        #                       `ContribPct` column). TAA's own row
        #                       reads 100% by construction.
        #   Max.Drawdown      — drawdown of the sleeve's growth-of-1.
        st.divider()
        st.markdown("### Scenario risk summary")
        st.caption(
            "Standalone vol, contribution to TAA vol (rebased to 100%), "
            "and max drawdown for each strategy in the **scenario book** "
            "(independent of the global Working book picker). Updates on "
            "every edit / add / prune. For the deeper analytics view, "
            "switch the Working book to **Scenario (editable)** and use "
            "the Risk tab."
        )
        ...
        if scn_strategy_returns.empty or scn_strategy_returns.shape[1] == 0:
            ...
            for col in scn_strategy_returns.columns:
                ...
                if col == TOTAL_COLUMN_NAME:
                    # TAA is the total — its share of itself is 100% by
                    # construction. Rendering it explicitly so the row
                    # is self-consistent.
                    rc_pct = 100.0
                elif not scn_contrib.empty and col in scn_contrib.index:
                    # `ContribPct` is already normalised to sum to 100%
                    # across strategies (see risk.compute_risk_contrib),
                    # so no extra rebasing needed here.
                    rc_pct = float(scn_contrib.loc[col, "ContribPct"])
                else:
                    rc_pct = float("nan")
                summary_rows.append({
                    "Strategy": col,
            ...
            # Strategies sorted by descending |contribution| so the
            # biggest movers come first; TAA pinned to the bottom as
            # the closing total row.
            taa_mask = summary_df["Strategy"] == TOTAL_COLUMN_NAME
            strat_part = (
                summary_df[~taa_mask]
                .assign(_abs_rc=lambda d: d["Risk Contrib %"].abs())
        ...
        if rv_df is None or rv_df.empty or rv_df.dropna(how="all").empty:
            ...
            with st.expander("2-strategy rolling relationship"):
                ...
                if len(pair_options) < 2:
                    ...
                    # Rolling correlation — inline so we don't drag in a
                    # new helper for one expression. `min_periods = w`
                    # matches the NaN philosophy of
                    # `risk.compute_rolling_vol` exactly.
                    w = int(rv_choice)
                    a_ret = pd.to_numeric(
                        strategy_returns[strat_a], errors="coerce",
                    )
                    b_ret = pd.to_numeric(
                        strategy_returns[strat_b], errors="coerce",
                    )
                    roll_corr = (
